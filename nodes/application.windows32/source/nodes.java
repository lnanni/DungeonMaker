    ...
    public void draw(){
        switch (this.action) {            
            ...
            case Triangulo:
                ...
                fill(0);
                textSize(23);
                text((char) index + "", pos.x-8, pos.y+17);
                fill(255);
...
class MapHandler {
  ...
  public void randomMap() {
    //place 4 tri
    add(new Action(ActionType.Triangulo, toGrid(50, height/2-50), 'W'));        
    add(new Action(ActionType.Triangulo, toGrid(width/2, 50), 'N'));        
    add(new Action(ActionType.Triangulo, toGrid(width/2, height-150), 'S'));
    add(new Action(ActionType.Triangulo, toGrid(width-50, height/2-50), 'E'));

    //place random circles;
    int  currentNodes = 0;
    noiseSeed(Instant.now().toEpochMilli());
    for (int i =150; i < width -100; i+=50) {
      for (int j = 150; j < height-200; j+=50) {
        float n = noise(i, j);
        if (n >= sliderValue && currentNodes < maxNodes) {
...
public void Toggle (boolean v){
 canLink = v;
}
public void slider(int v) {
   sliderValue = (v/100.0f);
}
public void nodes(int v) {
   maxNodes = v;
}
